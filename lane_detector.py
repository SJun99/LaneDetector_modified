...
class LineStringDetector:
    ...
    def _extrapolate_line(self, line_string: LineString, extend_len: int, stride: int) -> LineString:
        ...
        # 사용할 보간 방법 선택
        kind = 'cubic' if points.shape[0] >= 4 else 'linear'
        # x, y 좌표에 대해 interpolation 생성
        x_interp = interp1d(s, points[:, 0], kind=kind, fill_value="extrapolate")
        y_interp = interp1d(s, points[:, 1], kind=kind, fill_value="extrapolate")
    ...
    def _merge_lines(self, line_strings: List[LineString]) -> List[LineString]:
        ...
        for line in line_strings:
            ...
            for overlap_id in overlap_ids:
                ...
                ext_points = line.ext_points.reshape(-1, 1, 2)
                oppo_points = oppo_line.points.reshape(-1, 1, 2)

                cv2.polylines(combined_img, [ext_points], True, (255, 255, 255), 1)
                cv2.imshow('ext_points', combined_img)
                cv2.waitKey(0)

                cv2.polylines(combined_img, [oppo_points], True, (255, 255, 255), 1)
